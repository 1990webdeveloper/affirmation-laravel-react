...
@section('title', 'Binaural Beats')
@section('content')
    <!-- BEGIN: Content -->
    <div class="content">
        <div class="intro-y flex items-center mt-8">
            <h2 class="text-lg font-medium mr-auto">
                {{ $binauralBeat ? 'Binaural Beats Edit' : 'Binaural Beats Create' }}
            </h2>
        </div>
                ...
                <div class="intro-y box">
                    ...
                    <div id="vertical-form" class="p-5">
                        <form class="form-horizontal" method="POST"
                            action="{{ route('binaural-beats.store', $binauralBeat->id ?? '') }}"
                            enctype="multipart/form-data" id="bbForm">
                            @csrf
                            <div class="preview lg:w-1/2">
                                <div>
                                    ...
                                    <input id="vertical-form-1" type="text" name="name" autocomplete="off"
                                        ...
                                        value="{{ $binauralBeat ? $binauralBeat->name : old('name') }}">
                                </div>
                                @error('name')
                                ...
                                <div class="mt-3">
                                    <label for="vertical-form-2" class="form-label">Uplaod Audio File<span
                                            class="text-danger">*</span></label>
                                    @if (isset($binauralBeat))
                                        <input type="hidden" name="old_beats" id="old_beats" value="{{ $binauralBeat->audio_path }}">
                                    @endif

                                    <label class="block mb-5">
                                        <span class="sr-only">Choose binaural beats</span>
                                        <input type="file" name="audio_path" id="audioFile"
                                            ...
                                            value="{{ $binauralBeat ? $binauralBeat->audio_path : '' }}" />
                                        <span class="text-gray-400 mt-2 block">Recommended audio file size is:
                                            {{ $spaceLimit }} MB and {{ $timeLimit }} Minute</span>
                                    </label>
                                    @if (isset($binauralBeat))
                                        <audio class="mt-8" id="audioPreview"
                                            src="{{ asset('storage/binaural-beats/audio/' . $binauralBeat->audio_path) }}"
                                            controls></audio>
                                    @else
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="form-switch mt-2">
                                        <input type="checkbox" value='1' name="status" class="form-check-input"
                                            {{ $binauralBeat && $binauralBeat->status == '1' ? 'checked' : '' }}>
                                    </div>
                                </div>
        ...
        $(document).ready(function() {
            ...
            $('#bbForm').validate({
                ...
                rules: {
                    ...
                    audio_path: {
                        required: function() {
                            return $('#old_beats').val() == undefined ? true : false;
                        },
                ...
                messages: {
                    ...
                    audio_path: {
                        required: "Binaural beats is required.",
